...
Issues **#1–#5** (M0 — socle : ADR de stack, initialisation du dépôt, schéma de données, CI/CD,
environnements & secrets) livrées par le pipeline. **Prochaine étape : #6** (câblage du test gate
`MX_AGENT_TEST_CMD`) puis M1 (fonctionnalités — auth, salons, rendez-vous…).
