...
Issues **#1–#6** (M0 — socle : ADR de stack, initialisation du dépôt, schéma de données, CI/CD,
environnements & secrets, stratégie de tests & test gate) livrées par le pipeline.
**Prochaine étape : M1** (fonctionnalités — auth, salons, rendez-vous…).
