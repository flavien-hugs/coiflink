// Tests d'intégration — Route Handler BFF `PUT /api/salons/[id]/opening-hours`
// (#16). Couvre : validation domaine côté BFF, lecture de session (cookie
// httpOnly), mapping des réponses gateway → codes HTTP, absence de secret dans
// les réponses (PRD §11.3). `next/headers` et `next/server` sont mockés ;
// `fetch` est stubé globalement (frontière réseau).
...
import { PUT } from "../app/api/salons/[id]/opening-hours/route";
import { SESSION_COOKIE } from "../src/adapters/api/session-cookie-names";

const API_BASE = "http://api.test";

// Salon minimal renvoyé par le backend.
...
function makeRequest(body: unknown): Request {
  return new Request("http://localhost/api/salons/salon-abc/opening-hours", {
    method: "PUT",
...
// ---------------------------------------------------------------------------
// Validation domaine côté BFF
// ---------------------------------------------------------------------------

describe("PUT /api/salons/[id]/opening-hours — validation domaine", () => {
  it("horaires valides + session valide + backend 200 → 200 avec salon", async () => {
    withSession();
  ...
  it("horaires invalides (chevauchement) → 422 sans appel backend", async () => {
    withSession();
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);

    const res = await PUT(makeRequest(INVALID_BODY_OVERLAP), makeContext());
    expect(res.status).toBe(422);
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("weekly vide (non-vacuité) → 422 sans appel backend", async () => {
    withSession();
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);

    const res = await PUT(makeRequest({ weekly: {} }), makeContext());
    expect(res.status).toBe(422);
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("corps JSON invalide → 400", async () => {
    withSession();
    const request = new Request("http://localhost/api/salons/abc/opening-hours", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: "not json{{{",
    });
...
describe("PUT /api/salons/[id]/opening-hours — session", () => {
  ...
  it("la réponse 401 ne contient pas le jeton", async () => {
    withoutSession();
...
describe("PUT /api/salons/[id]/opening-hours — mapping gateway", () => {
  ...
  it("backend 422 → BFF 422", async () => {
    withSession();
    stubBackend(422, { detail: "Structure invalide." });
    const res = await PUT(makeRequest(VALID_BODY), makeContext());
    expect(res.status).toBe(422);
  });
...
describe("PUT /api/salons/[id]/opening-hours — sécurité", () => {
  ...
  it("la réponse 200 contient le salon (pas de données brutes du backend non filtrées)", async () => {
    withSession();
    stubBackend(200, FAKE_SALON_PAYLOAD);

    const res = await PUT(makeRequest(VALID_BODY), makeContext());
    const body = await res.json();
    expect(body).toHaveProperty("salon");
    expect(body.salon).toHaveProperty("id");
  });
});
