"""Port de persistance des **bornes kiosque** (`Protocol`, US-8.1, #155).

Le cas d'usage de provisioning (`application/kiosk_devices.py`) et
l'authentification du device (`application/kiosk_authentication.py`) déclarent ici
leurs besoins ; l'implémentation SQLAlchemy vit dans
`adapters/outbound/persistence/kiosk_device_repository.py`.

**Encapsulation (décision d'architecture d'ADR-0041)** : ce port cache que la
borne vit comme un compte de service `users` (rôle `KIOSK`) + un rattachement
...
class KioskDeviceRepository(Protocol):
    """Contrat de persistance des bornes kiosque (compte de service encapsulé)."""

    def create(self, device: KioskDeviceToCreate) -> KioskDevice:
        """Provisionne une borne (compte `users` `KIOSK` + rattachement `salon_members`).
    ...
    def find_by_id(
        ...
        Le cas d'usage lève alors `KioskDeviceNotFound` (`404` **après** la portée).
        """
        ...
    ...
    def find_credentials(
        ...
        résout **que** les comptes de service `KIOSK` : un `device_id` pointant un
