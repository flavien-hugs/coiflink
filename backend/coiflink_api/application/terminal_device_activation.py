"""Cas d'usage : activation d'une borne terminal par code à 6 chiffres (US-8.1, #155 — provisioning silencieux).
...
credential longue durée, puis utilise `POST /auth/terminal/login` **inchangé** pour
...
jamais journalisé, jamais relisible ») : au provisioning, `ProvisionTerminalDevice` ne
pose qu'un condensat **placeholder** (secret jetable aussitôt oublié), donc
inutilisable — `/auth/terminal/login` échoue déterministiquement sur une borne non
...
# que ProvisionTerminalDevice, juste déclenché par l'activation plutôt que la création.
_SECRET_NBYTES = 32
...
class ActivateTerminalDeviceCommand:
...
@dataclass(frozen=True)
class ActivateTerminalDeviceResult:
...
class ActivateTerminalDevice:
    ...
    def execute(self, command: ActivateTerminalDeviceCommand) -> ActivateTerminalDeviceResult:
        ...
        le défi est supprimé — la borne peut se connecter via `/auth/terminal/login`, et
        ...
        key = f"terminal-activate|{command.client_ip or '-'}"
        ...
        return ActivateTerminalDeviceResult(device_id=device_id, secret=secret)
