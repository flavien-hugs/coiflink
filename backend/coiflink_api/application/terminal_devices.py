"""Cas d'usage : **provisioning des bornes terminal** par un gérant (US-8.1, #155).

Orchestre la création, la lecture et la révocation d'une **borne terminal** —
...
`TERMINAL` + rattachement `salon_members`, atomiques) → **émettre un code d'activation
à 6 chiffres** (défi `OtpChallenge`, TTL 24 h, 5 essais) → **journaliser**
`TERMINAL_DEVICE_PROVISIONED` (`metadata = {}`, ni code ni condensat ni libellé) →
...
(`application/terminal_device_activation.py::ActivateTerminalDevice`), quand la borne
...
correspond à aucun secret connu, `/auth/terminal/login` échoue déterministiquement).

**Révocation** — suspension logique (jamais une suppression, traçabilité §11.4) :
audit `TERMINAL_DEVICE_REVOKED`, effet immédiat (relecture du statut par requête).
"""
...
from coiflink_api.domain.errors import TerminalDeviceNotFound
...
class ProvisionTerminalDeviceCommand:
...
class ProvisionTerminalDevice:
    ...
    def __init__(
        ...
        audit_log: AuditLog,
        activation_repository: TerminalActivationRepository,
        *,
    ...
    def execute(
        self,
        salon_id: uuid.UUID,
        command: ProvisionTerminalDeviceCommand,
        *,
        actor_user_id: uuid.UUID,
    ) -> tuple[TerminalDevice, str]:
        ...
        secret longue durée via `ActivateTerminalDevice` (`POST /auth/terminal/activate`),
        ...
        **placeholder** (secret jetable, immédiatement oublié) : il ne correspond à
        ...
        self._audit_log.record(
            AuditEntry(
                action=AuditAction.TERMINAL_DEVICE_PROVISIONED.value,
                actor_user_id=actor_user_id,
                salon_id=salon_id,
                entity_type=ENTITY_TYPE_TERMINAL_DEVICE,
                entity_id=device.id,
                metadata={},
            )
        )
...
class ListTerminalDevices:
    """Liste les bornes **du salon** (lecture gérant — aucune écriture, aucun audit)."""

    def __init__(self, repository: TerminalDeviceRepository) -> None:
        self._repository = repository

    def execute(self, salon_id: uuid.UUID) -> tuple[TerminalDevice, ...]:
        return self._repository.list_for_salon(salon_id)


class RevokeTerminalDevice:
    """Révoque une borne **du salon** (suspension logique) et journalise (§11.4)."""

    def __init__(
        self, repository: TerminalDeviceRepository, audit_log: AuditLog
    ) -> None:
        self._repository = repository
        self._audit_log = audit_log
    ...
    def execute(
        ...
        *,
        ...
        """Suspend la borne (effet immédiat) ; `TerminalDeviceNotFound` si hors salon."""

        device = self._repository.revoke(salon_id, device_id)
        if device is None:
            # Borne inexistante **ou** hors salon : indiscernables (§11.2).
            raise TerminalDeviceNotFound("Borne introuvable.")
        self._audit_log.record(
            AuditEntry(
                action=AuditAction.TERMINAL_DEVICE_REVOKED.value,
                actor_user_id=actor_user_id,
                salon_id=salon_id,
                entity_type=ENTITY_TYPE_TERMINAL_DEVICE,
                entity_id=device.id,
