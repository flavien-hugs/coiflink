"""Cas d'usage : **authentification d'une borne kiosque** (US-8.1, #155).

Échange le credential de device longue durée `(device_id, secret)` — stocké côté
borne (#159/#161) — contre une paire JWT **standard et courte** au rôle `KIOSK`.
...
_DUMMY_SECRET = "dummy-kiosk-secret-for-timing-mitigation"


@dataclass(frozen=True)
class KioskLoginCommand:
...
class AuthenticateKioskDevice:
    """Cas d'usage : credential de device valide → `KioskLoginResult`, sinon lève."""
    ...
    def execute(self, command: KioskLoginCommand) -> KioskLoginResult:
        ...
        tokens = self._token_service.issue_pair(creds.id, Role.KIOSK.value)
        return KioskLoginResult(tokens=tokens, salon_id=creds.salon_id)

    def _find(self, raw_device_id: str) -> KioskDeviceCredentials | None:
    ...
    def _verify_secret(
        self, creds: KioskDeviceCredentials | None, secret: str
    ) -> bool:
