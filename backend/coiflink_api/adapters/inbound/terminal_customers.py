"""Adapter entrant (driving) : router HTTP **borne terminal — identité walk-in** (US-8.2, #156).

Expose au rôle `TERMINAL` (compte de service d'une borne, ADR-0041) les deux
opérations d'identité du parcours « client sans rendez-vous » (PRD §17), sous
`/salons/{salon_id}/terminal/customers[...]`, imbriquées sous le salon pour hériter de
...
- chaque route déclare `require_salon_scope` **et** la permission `TERMINAL` **dédiée**
  déjà livrée par #155 (`CUSTOMER_LOOKUP_TERMINAL` / `CUSTOMER_CREATE_WALKIN`) : la
  ...
  générique) sur ces routes ; un credential `TERMINAL` reste incapable d'atteindre
...
from coiflink_api.application.terminal_customers import (
...
router = APIRouter(prefix="/salons", tags=["terminal"])
...
# Message **générique** d'anti-énumération (patron `/auth/terminal/login`, #155).
...
# --------------------------------------------------------------------------- #
class TerminalLookupRequest(BaseModel):
    """Corps de `POST /salons/{salon_id}/terminal/customers/lookup`.
...
# --------------------------------------------------------------------------- #
def get_terminal_lookup_rate_limiter(request: Request) -> LoginRateLimiter:
    """Limiteur anti-énumération **dédié au lookup terminal** (singleton `app.state`).

    Réutilise le port `LoginRateLimiter` et l'adapter `InMemoryLoginRateLimiter`
    exactement comme #155 pour `/auth/terminal/login` — mais avec un **singleton
    distinct** (`terminal_lookup_rate_limiter`) et des seuils propres au lookup :
    verrouiller les recherches d'une borne ne doit pas verrouiller son login (et
    inversement). Repli sûr avec les seuils terminal d'`AuthConfig` si l'état n'est
    pas configuré (tests isolés).
    """

    limiter = getattr(request.app.state, "terminal_lookup_rate_limiter", None)
    if limiter is None:
        ...
        limiter = InMemoryLoginRateLimiter(
            ...
            lockout=config.terminal_lookup_lockout,
        )
        request.app.state.terminal_lookup_rate_limiter = limiter
    return limiter
...
# --------------------------------------------------------------------------- #
@router.post(
    "/{salon_id}/terminal/customers/lookup",
...
def lookup_walk_in_customer(
    ...
    principal: Annotated[
        Principal, Depends(require_permission(Permission.CUSTOMER_LOOKUP_TERMINAL))
    ],
...
__all__ = [
    "router",
    "TerminalLookupRequest",
    "TerminalWalkInCreateRequest",
    "WalkInIdentityResponse",
