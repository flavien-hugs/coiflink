"""Adapter sortant : persistance des **bornes kiosque** (SQLAlchemy, US-8.1, #155).

Implémente le port `KioskDeviceRepository`. **Encapsule** le fait qu'une borne est,
en base, un **compte de service** `users` (rôle `KIOSK`) + un rattachement
...
class SqlKioskDeviceRepository:
    """Dépôt de bornes kiosque adossé à une `Session` SQLAlchemy (compte de service)."""

    def __init__(self, session: Session) -> None:
        self._session = session

    def create(self, device: KioskDeviceToCreate) -> KioskDevice:
        """Insère le compte de service `KIOSK` + son rattachement `salon_members`.
    ...
    def list_for_salon(self, salon_id: uuid.UUID) -> tuple[KioskDevice, ...]:
        """Liste les bornes du salon (jointure `salon_members` `KIOSK` ↔ `users`)."""
        ...
        rows = self._session.execute(
            ...
            .where(
                ...
                models.SalonMember.role == Role.KIOSK.value,
            )
    ...
    def find_credentials(
    ...
    ) -> KioskDeviceCredentials | None:
        """Charge le credential d'une borne (scopé `device_id`, rôle `KIOSK` exigé)."""
        ...
        row = self._session.execute(
            ...
            .where(
                models.User.id == device_id,
                models.User.role == Role.KIOSK.value,
                models.SalonMember.role == Role.KIOSK.value,
            )
            .limit(1)
        ).first()
    ...
    def revoke(
        ...
        n'existe pas dans ce salon (le cas d'usage lève `KioskDeviceNotFound`).
        """
...
def _to_device(salon_id: uuid.UUID, user: models.User) -> KioskDevice:
    """Mappe une ligne `users` (compte de service `KIOSK`) vers `KioskDevice`.

    Ne recopie **jamais** `password_hash` : `KioskDevice` est la vue exposée au
...
__all__ = ["SqlKioskDeviceRepository"]
