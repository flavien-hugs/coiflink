"""Tests des cas d'usage borne terminal — identité walk-in (US-8.2, #156).
...
def test_terminal_customers_module_does_not_import_user_repository() -> None:
    """Le module applicatif terminal n'importe **jamais** le port `users` (ADR-0026).
    ...
    """
    mod = importlib.import_module("coiflink_api.application.terminal_customers")
    # Vérifie toutes les importations directes connues du module.
    assert not hasattr(mod, "UserRepository"), (
        "terminal_customers importe UserRepository — oracle ADR-0026 violé"
    )
    ...
    assert "user_repository" not in source, (
        "terminal_customers contient 'user_repository' dans son code source — ADR-0026 violé"
    )
