"""Tests unitaires — domaine `terminal_device` (US-8.1, #155).
...
- `TerminalDevice` : champs attendus, gelé, **sans** champ secret ni condensat (invariant
  de non-fuite §11.3 : la vue exposée au gérant ne porte jamais le secret) ;
- `TerminalDeviceCredentials` : porte `password_hash` (interne, jamais sérialisé) ;
- `TerminalDeviceToCreate` : construction correcte, gelé, sans champ secret en clair.
...
class TestValidateDeviceLabel:
    ...
    def test_error_message_is_neutral_not_empty(self) -> None:
        ...
        with pytest.raises(InvalidTerminalDeviceLabel) as exc_info:
            validate_device_label("")
...
# ---------------------------------------------------------------------------
# TerminalDevice — entité publique (sans secret, invariant §11.3)
# ---------------------------------------------------------------------------


def _make_device(**kwargs) -> TerminalDevice:  # type: ignore[no-untyped-def]
    defaults = dict(
        id=_DEVICE_ID,
        salon_id=_SALON_ID,
        label="Borne entrée",
    ...
    return TerminalDevice(**defaults)


class TestTerminalDevice:
    ...
    def test_has_no_password_hash_field(self) -> None:
        ...
        field_names = {f.name for f in fields(TerminalDevice)}
...
# ---------------------------------------------------------------------------
# TerminalDeviceCredentials — interne, jamais sérialisé (§11.3)
# ---------------------------------------------------------------------------


def _make_credentials(**kwargs) -> TerminalDeviceCredentials:  # type: ignore[no-untyped-def]
    defaults = dict(
        id=_DEVICE_ID,
        salon_id=_SALON_ID,
    ...
    defaults.update(kwargs)
    return TerminalDeviceCredentials(**defaults)


class TestTerminalDeviceCredentials:
    def test_carries_password_hash_field(self) -> None:
        """Interne : porte le condensat nécessaire à `PasswordHasher.verify` (login borne)."""
        field_names = {f.name for f in fields(TerminalDeviceCredentials)}
...
class TestTerminalDeviceToCreate:
    def test_construction_nominal(self) -> None:
        to_create = TerminalDeviceToCreate(
            salon_id=_SALON_ID,
            label="Borne test",
    ...
    def test_has_no_plaintext_secret_field(self) -> None:
        """Invariant : le secret en clair ne doit jamais transiter par `TerminalDeviceToCreate`."""
        field_names = {f.name for f in fields(TerminalDeviceToCreate)}
        assert "secret" not in field_names
