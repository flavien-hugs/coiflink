"""Tests unitaires — `AuthenticateKioskDevice` (US-8.1, #155).

Couvre l'orchestration du cas d'usage sans I/O réelle :
- Parcours heureux : credential valide → paire JWT rôle `KIOSK` + `salon_id`
...
from coiflink_api.domain.kiosk_device import KioskDeviceCredentials
...
class _FakeKioskRepo:
    """Dépôt en mémoire : un seul credential configurable, aucun secret réel."""

    def __init__(self, creds: KioskDeviceCredentials | None = None) -> None:
        self._creds = creds
        self.find_calls: list[uuid.UUID] = []

    def find_credentials(self, device_id: uuid.UUID) -> KioskDeviceCredentials | None:
...
def _make_usecase(
    *,
    creds: KioskDeviceCredentials | None,
    rate_limiter: FakeLoginRateLimiter | None = None,
    token_service: FakeTokenService | None = None,
) -> tuple[AuthenticateKioskDevice, _FakeKioskRepo, FakeLoginRateLimiter]:
    repo = _FakeKioskRepo(creds)
...
class TestAuthenticateKioskDeviceSuccess:
    def test_returns_kiosk_login_result(self) -> None:
        uc, _repo, _limiter = _make_usecase(creds=_creds())
        result = uc.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET))
        assert isinstance(result, KioskLoginResult)
    ...
    def test_token_issued_with_kiosk_role(self) -> None:
        """Le jeton est émis avec `role = KIOSK` — jamais avec un rôle humain."""
        ts = FakeTokenService()
        uc, _repo, _limiter = _make_usecase(creds=_creds(), token_service=ts)
        uc.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET))
        assert len(ts.issued) == 1
        _uid, issued_role = ts.issued[0]
        assert issued_role == Role.KIOSK.value
    ...
    def test_device_id_with_surrounding_spaces_is_normalised(self) -> None:
        ...
        uc, _repo, _limiter = _make_usecase(creds=_creds())
        result = uc.execute(
            KioskLoginCommand(device_id=f"  {_DEVICE_ID}  ", secret=_GOOD_SECRET)
        )
        assert result.salon_id == _SALON_ID
...
class TestAuthenticateKioskDeviceFailure:
    ...
    def test_wrong_secret_raises_invalid_credentials(self) -> None:
        uc, _repo, _limiter = _make_usecase(creds=_creds())
        with pytest.raises(InvalidCredentials):
            uc.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret="wrong-secret"))
    ...
    def test_non_uuid_device_id_raises_invalid_credentials(self) -> None:
        ...
        uc, _repo, _limiter = _make_usecase(creds=_creds())
        with pytest.raises(InvalidCredentials):
            uc.execute(KioskLoginCommand(device_id="not-a-uuid", secret=_GOOD_SECRET))

    def test_empty_device_id_raises_invalid_credentials(self) -> None:
        uc, _repo, _limiter = _make_usecase(creds=_creds())
        with pytest.raises(InvalidCredentials):
            uc.execute(KioskLoginCommand(device_id="", secret=_GOOD_SECRET))
    ...
    def test_unknown_device_message_equals_wrong_secret_message(self) -> None:
        ...
        with pytest.raises(InvalidCredentials) as ei_unknown:
            uc_unknown.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET))
        with pytest.raises(InvalidCredentials) as ei_wrong:
            uc_wrong.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret="bad"))
    ...
    def test_suspended_message_equals_wrong_secret_message(self) -> None:
        ...
        with pytest.raises(InvalidCredentials) as ei_susp:
            uc_susp.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET))
        with pytest.raises(InvalidCredentials) as ei_wrong:
            uc_wrong.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret="bad"))
    ...
    def test_unknown_device_triggers_dummy_verify_not_impl_error(self) -> None:
        ...
        uc, _repo, _limiter = _make_usecase(creds=None)
        with pytest.raises(InvalidCredentials):
            uc.execute(KioskLoginCommand(device_id=str(_DEVICE_ID), secret="anything"))
...
class TestKioskRateLimiter:
    ...
    def test_rate_limiter_key_includes_ip(self) -> None:
        ...
        client_ip = "10.0.0.42"
        uc.execute(
            KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET, client_ip=client_ip)
        )
    ...
    def test_rate_limiter_key_without_ip_uses_dash(self) -> None:
        uc, _repo, limiter = _make_usecase(creds=_creds())
        uc.execute(
            KioskLoginCommand(device_id=str(_DEVICE_ID), secret=_GOOD_SECRET, client_ip=None)
        )
