"""Tests unitaires — `ActivateTerminalDevice` (US-8.1, #155).
...
) -> tuple[ActivateTerminalDevice, _FakeActivationRepo, _FakeDeviceRepo, FakeLoginRateLimiter]:
...
class TestActivateTerminalDeviceSuccess:
    def test_returns_activation_result(self) -> None:
        uc, _act, _dev, _lim = _make_usecase()
        result = uc.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE))
        assert isinstance(result, ActivateTerminalDeviceResult)
    ...
    def test_activation_challenge_deleted_after_success(self) -> None:
        ...
        uc.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE))
    ...
    def test_code_with_surrounding_spaces_is_normalised(self) -> None:
        uc, _act, _dev, _lim = _make_usecase()
        result = uc.execute(ActivateTerminalDeviceCommand(code=f"  {_GOOD_CODE}  "))
        assert result.device_id == _DEVICE_ID
...
class TestActivateTerminalDeviceFailure:
    ...
    def test_wrong_code_raises_invalid_activation_code(self) -> None:
        uc, _act, _dev, _lim = _make_usecase()
        with pytest.raises(InvalidActivationCode):
            uc.execute(ActivateTerminalDeviceCommand(code="999999"))

    def test_empty_code_raises_invalid_activation_code(self) -> None:
        uc, _act, _dev, _lim = _make_usecase()
        with pytest.raises(InvalidActivationCode):
            uc.execute(ActivateTerminalDeviceCommand(code=""))
    ...
    def test_unknown_and_expired_messages_are_identical(self) -> None:
        ...
        with pytest.raises(InvalidActivationCode) as ei_unknown:
            uc_unknown.execute(ActivateTerminalDeviceCommand(code="000000"))
        with pytest.raises(InvalidActivationCode) as ei_expired:
            uc_expired.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE))
...
class TestActivationRateLimiter:
    ...
    def test_rate_limiter_key_uses_client_ip(self) -> None:
        ...
        uc.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE, client_ip="10.0.0.42"))
        assert len(limiter.checks) == 1
        assert "10.0.0.42" in limiter.checks[0]

    def test_rate_limiter_key_without_ip_uses_dash(self) -> None:
        uc, _act, _dev, limiter = _make_usecase()
        uc.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE, client_ip=None))
    ...
    def test_rate_limiter_key_does_not_depend_on_device_id(self) -> None:
        ...
        uc, _act, _dev, limiter = _make_usecase()
        uc.execute(ActivateTerminalDeviceCommand(code=_GOOD_CODE, client_ip="1.2.3.4"))
