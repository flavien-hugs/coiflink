"""Tests unitaires — cas d'usage provisioning/révocation bornes terminal (US-8.1, #155).

Couvre `ProvisionTerminalDevice`, `ListTerminalDevices` et `RevokeTerminalDevice` :

**Provisioning** (depuis #155 « provisioning silencieux ») :
- Retourne l'entité **et** le **code d'activation à 6 chiffres** (une seule fois) —
  plus jamais le secret : celui-ci est généré à l'activation (`ActivateTerminalDevice`).
...
- Audit journalisé `TERMINAL_DEVICE_PROVISIONED`, `entity_type = terminal_device`,
  `metadata = {}` (invariant §11.3/§11.4 : ni secret, ni condensat, ni libellé).
- Label invalide (vide, trop long) → `InvalidTerminalDeviceLabel`, **aucun** audit.

**Listing** : délégation directe au dépôt (lecture pure, pas d'audit).

**Révocation** :
- Suspension logique journalisée `TERMINAL_DEVICE_REVOKED`, `entity_type = terminal_device`,
  `metadata = {}` (même invariant que le provisioning).
- Borne hors salon / inexistante → `TerminalDeviceNotFound` (aucun oracle §11.2).
...
class _FakeTerminalDeviceRepo:
    """Dépôt en mémoire : create / list / revoke sans I/O réelle."""

    def __init__(self) -> None:
        self.created: list[TerminalDeviceToCreate] = []
        self._devices: dict[uuid.UUID, TerminalDevice] = {}
    ...
    def revoke(self, salon_id: uuid.UUID, device_id: uuid.UUID) -> TerminalDevice | None:
...
class _FakeTerminalActivationRepo:
...
class TestProvisionTerminalDevice:
    def _make(
        self,
        audit_log: FakeAuditLog | None = None,
        activation_repository: _FakeTerminalActivationRepo | None = None,
        rng: Random | None = None,
    ) -> tuple[ProvisionTerminalDevice, _FakeTerminalDeviceRepo, FakeAuditLog, _FakeTerminalActivationRepo]:
        repo = _FakeTerminalDeviceRepo()
        log = audit_log or FakeAuditLog()
        activation_repo = activation_repository or _FakeTerminalActivationRepo()
        uc = ProvisionTerminalDevice(
            repository=repo,
            hasher=_HASHER,
            audit_log=log,
    ...
    def _execute(self, uc: ProvisionTerminalDevice, label: str = "Borne entrée") -> tuple[TerminalDevice, str]:
        return uc.execute(
            _SALON_ID,
            ProvisionTerminalDeviceCommand(label=label),
    ...
    def test_returned_device_has_no_attribute_password_hash(self) -> None:
        """Invariant §11.3 : `TerminalDevice` retourné ne porte jamais le condensat."""
        uc, _repo, _log, _act = self._make()
        device, _secret = self._execute(uc)
    ...
    def test_invalid_empty_label_raises_before_audit(self) -> None:
        ...
        with pytest.raises(InvalidTerminalDeviceLabel):
            uc.execute(
                _SALON_ID, ProvisionTerminalDeviceCommand(label=""),
                actor_user_id=_ACTOR_ID,
            )
    ...
    def test_audit_action_is_terminal_device_provisioned(self) -> None:
        uc, _repo, log, _act = self._make()
        self._execute(uc)
        assert log.recorded[0].action == AuditAction.TERMINAL_DEVICE_PROVISIONED.value

    def test_audit_entity_type_is_terminal_device(self) -> None:
        uc, _repo, log, _act = self._make()
        self._execute(uc)
        assert log.recorded[0].entity_type == ENTITY_TYPE_TERMINAL_DEVICE
...
class TestListTerminalDevices:
    def test_returns_devices_for_salon(self) -> None:
        repo = _FakeTerminalDeviceRepo()
        repo.create(TerminalDeviceToCreate(salon_id=_SALON_ID, label="Borne A", password_hash="h"))
        uc = ListTerminalDevices(repository=repo)
...
class TestRevokeTerminalDevice:
    def _provisioned_repo(self) -> _FakeTerminalDeviceRepo:
        """Repo avec une borne pré-provisionnée dans `_SALON_ID`."""
        repo = _FakeTerminalDeviceRepo()
        repo.create(TerminalDeviceToCreate(salon_id=_SALON_ID, label="Borne test", password_hash="h"))
        return repo

    def _make(
        self,
        repo: _FakeTerminalDeviceRepo | None = None,
        log: FakeAuditLog | None = None,
    ) -> tuple[RevokeTerminalDevice, _FakeTerminalDeviceRepo, FakeAuditLog]:
        _repo = repo or self._provisioned_repo()
        _log = log or FakeAuditLog()
        uc = RevokeTerminalDevice(repository=_repo, audit_log=_log)
    ...
    def test_audit_action_is_terminal_device_revoked(self) -> None:
        uc, _repo, log = self._make()
        uc.execute(_SALON_ID, _DEVICE_ID, actor_user_id=_ACTOR_ID)
        assert log.recorded[0].action == AuditAction.TERMINAL_DEVICE_REVOKED.value
    ...
    def test_unknown_device_raises_terminal_device_not_found(self) -> None:
        """Borne inexistante → `TerminalDeviceNotFound` (après portée, §11.2)."""
        repo = _FakeTerminalDeviceRepo()  # aucune borne provisionnée
        uc, _repo, _log = self._make(repo=repo)
        with pytest.raises(TerminalDeviceNotFound):
            uc.execute(_SALON_ID, _DEVICE_ID, actor_user_id=_ACTOR_ID)

    def test_device_in_other_salon_raises_terminal_device_not_found(self) -> None:
        """Borne d'un autre salon → indiscernable d'une borne inexistante (§11.2)."""
        repo = _FakeTerminalDeviceRepo()
        repo.create(TerminalDeviceToCreate(salon_id=_OTHER_SALON, label="Borne B", password_hash="h"))
        uc, _repo, _log = self._make(repo=repo)
        with pytest.raises(TerminalDeviceNotFound):
