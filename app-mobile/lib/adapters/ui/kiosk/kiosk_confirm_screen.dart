...
import 'kiosk_ticket_number_screen.dart';
...
class _KioskConfirmScreenState extends State<KioskConfirmScreen> {
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute<void>(
          builder: (_) => KioskTicketNumberScreen(
            deps: widget.deps,
            ticket: ticket,
    ...
    } on KioskQueueException catch (exc) {
      if (!mounted) return;
      setState(() {
        _error = exc.message;
        _submitting = false;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: KioskColors.border, width: 1.5),
      ),
  ...
  Widget _row(BuildContext context, String label, String value) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: <Widget>[
          Text(label, style: theme.textTheme.bodyMedium?.copyWith(color: KioskColors.muted)),
          Flexible(
            child: Text(
              value,
