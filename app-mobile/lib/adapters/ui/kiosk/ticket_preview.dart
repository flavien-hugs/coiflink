...
// `adapters/ui/kiosk/ticket_preview.dart` est celui attendu par les deux specs.
...
String formatKioskIssuedAt(DateTime issuedAt) {
  final date = '${_two(issuedAt.day)}/${_two(issuedAt.month)}/${issuedAt.year}';
  return '$date ${formatKioskTime(issuedAt)}';
}
...
class TicketPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ConstrainedBox(
      ...
      child: CustomPaint(
        painter: const _DashedRectPainter(color: KioskColors.border),
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              _receiptLine('N°', formatKioskTicketNumber(payload.ticketNumber)),
              _receiptLine('Date', formatKioskIssuedAt(payload.issuedAt)),
...
class _DashedDivider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const SizedBox(
      height: 1,
      child: CustomPaint(painter: _DashedLinePainter(color: KioskColors.border)),
    );
  }
}
