...
class KioskNumericKeypad extends StatelessWidget {
  ...
  Widget _row(List<Widget> children) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: KioskDimensions.touchSpacing / 2),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: <Widget>[
          for (var i = 0; i < children.length; i++) ...<Widget>[
            if (i > 0) const SizedBox(width: KioskDimensions.touchSpacing),
            children[i],
          ],
        ],
      ),
  ...
  Widget _digitKey(String digit) {
    return Expanded(
      child: SizedBox(
        height: KioskDimensions.primaryButtonHeight,
