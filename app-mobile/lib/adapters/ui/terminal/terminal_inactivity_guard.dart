...
// `builder:`, voir `terminal_app.dart`), il ramène systématiquement à l'accueil après
...
abstract class TerminalInactivityController {
  ...
  /// gérant à la sortie du terminal) — **sans** plafond : un gérant ne doit pas être
...
class TerminalInactivityGuard extends StatefulWidget {
  ...
  /// Contrôleur du guard le plus proche, ou `null` hors d'un `TerminalInactivityGuard`.
  static TerminalInactivityController? maybeOf(BuildContext context) {
    return context
        .dependOnInheritedWidgetOfExactType<_TerminalInactivityScope>()
        ?.controller;
  }
}

class TerminalInactivityGuardState extends State<TerminalInactivityGuard>
    implements TerminalInactivityController {
  Timer? _timer;
  ...
  Widget build(BuildContext context) {
    return Listener(
      ...
      child: _TerminalInactivityScope(controller: this, child: widget.child),
    );
  }
}

/// Rend le contrôleur du guard accessible à tout écran poussé sous le `Navigator`.
class _TerminalInactivityScope extends InheritedWidget {
  const _TerminalInactivityScope({
    required this.controller,
    required super.child,
  });

  final TerminalInactivityController controller;

  @override
  bool updateShouldNotify(_TerminalInactivityScope oldWidget) =>
