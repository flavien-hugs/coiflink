// Tests widget — KioskInactivityGuard (US-8.5, #159).
//
...
// ---------------------------------------------------------------------------
// Helper : construit un MaterialApp avec KioskInactivityGuard dans builder:
// ---------------------------------------------------------------------------

Widget _buildApp({
  required GlobalKey<NavigatorState> navKey,
  GlobalKey<KioskInactivityGuardState>? guardKey,
...
void main() {
  group('KioskInactivityGuard — retour automatique', () {
  ...
  group('KioskInactivityGuard — pauseForPrinting', () {
    testWidgets('plafond déclenche le retour si resumeAfterPrinting n\'est pas appelé',
        (tester) async {
      final navKey = GlobalKey<NavigatorState>();
      final guardKey = GlobalKey<KioskInactivityGuardState>();
  ...
  group('KioskInactivityGuard — pauseForModal', () {
    ...
    testWidgets('retourne le contrôleur depuis un widget enfant', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Builder(
            builder: (context) {
              captured = KioskInactivityGuard.maybeOf(context);
      ...
      expect(captured, isA<KioskInactivityController>());
    });

    testWidgets('retourne null hors d\'un KioskInactivityGuard', (tester) async {
      KioskInactivityController? captured;
  ...
  group('KioskInactivityGuard — onPointerMove', () {
  ...
  group('KioskInactivityGuard — suspension pendant l\'impression', () {
    testWidgets(
