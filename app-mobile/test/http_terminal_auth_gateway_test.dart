// Tests unitaires — HttpTerminalAuthGateway : mapping JSON → domaine (US-8.1, #155).
//
// Couverture : mapping access_token/salon_id (refresh_token ignoré s'il est
// présent) ; 401 → TerminalInvalidCredentialException ; 5xx/429 → TerminalAuthException
// générique ; panne réseau → TerminalAuthException ; corps illisible →
// TerminalAuthException ; device_id/secret transmis dans le corps (jamais l'URL).
...
import 'package:coiflink_mobile/adapters/data/http_terminal_auth_gateway.dart';
...
HttpTerminalAuthGateway _gateway(http.Client client) =>
    HttpTerminalAuthGateway(config: _config(), client: client);
...
void main() {
  group('HttpTerminalAuthGateway — succès', () {
  ...
  group('HttpTerminalAuthGateway — erreurs HTTP', () {
    test('401 → TerminalInvalidCredentialException', () async {
    ...
    test('500 → TerminalAuthException générique (pas Invalid)', () async {
      final client = _FakeHttpClient(statusCode: 500, body: '');

      await expectLater(
        _gateway(client).login(_credential),
        throwsA(
          allOf(isA<TerminalAuthException>(), isNot(isA<TerminalInvalidCredentialException>())),
        ),
      );
    });

    test('429 → TerminalAuthException générique', () async {
  ...
  group('HttpTerminalAuthGateway — pannes réseau et de parsing', () {
    test('panne réseau → TerminalAuthException', () async {
      await expectLater(
    ...
    test('corps JSON illisible → TerminalAuthException', () async {
