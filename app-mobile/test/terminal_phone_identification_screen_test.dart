// Tests widget — TerminalPhoneIdentificationScreen (US-002, #159).
//
...
// manuelle ; navigation vers TerminalCreateCustomerScreen sur client absent ; message
// d'erreur neutre sur TerminalIdentityException ; correction (backspace).
//
// Respecte le choix de pavé numérique interne (TerminalNumericKeypad) : aucun
...
TerminalDeps _makeDeps(TerminalIdentityGateway identityGateway) => TerminalDeps(
      salonId: 'salon-1',
...
Widget _buildScreen(TerminalDeps deps) =>
    MaterialApp(home: TerminalPhoneIdentificationScreen(deps: deps));
...
void main() {
  group('TerminalPhoneIdentificationScreen — saisie', () {
  ...
  group('TerminalPhoneIdentificationScreen — client trouvé', () {
    ...
    testWidgets(
        'enchaîne automatiquement vers TerminalServiceSelectionScreen, sans confirmation manuelle',
        (tester) async {
      _setTabletSurface(tester);
      final gateway = _StubIdentityGateway(
  ...
  group('TerminalPhoneIdentificationScreen — client absent', () {
    testWidgets('navigation vers TerminalCreateCustomerScreen si null retourné',
        (tester) async {
      _setTabletSurface(tester);
  ...
  group('TerminalPhoneIdentificationScreen — erreur réseau', () {
    testWidgets('affiche un message d\'erreur neutre sur TerminalIdentityException',
        (tester) async {
      _setTabletSurface(tester);
      final gateway = _StubIdentityGateway(
    ...
    testWidgets('aucune navigation automatique sur erreur (toujours en direct)',
      ...
      expect(find.byType(TerminalCreateCustomerScreen), findsNothing);
    });
  });
}
