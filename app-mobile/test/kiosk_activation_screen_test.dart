// Tests widget — KioskActivationScreen (US-8.5, #159).
//
...
class _InMemoryCredentialStore implements KioskCredentialStore {
  KioskCredential? saved;

  @override
  Future<KioskCredential?> read() async => saved;

  @override
  Future<void> save(KioskCredential credential) async => saved = credential;
...
Widget _buildScreen({
  required KioskActivationGateway activationGateway,
  required KioskCredentialStore credentialStore,
  ValueChanged<KioskCredential>? onActivated,
}) {
  return MaterialApp(
...
void main() {
  group('KioskActivationScreen — saisie', () {
  ...
  group('KioskActivationScreen — succès', () {
    testWidgets('code complet → active, sauvegarde le credential et appelle onActivated',
      ...
      // `KioskBootstrap`) — l'indicateur de chargement resterait affiché indéfiniment
...
class _FlakyActivationGateway implements KioskActivationGateway {
  ...
  Future<KioskCredential> activate(String code) async {
    calls.add(code);
    if (onFirstCall()) throw const KioskActivationException();
    return credential;
  }
}
