// Tests widget — showTerminalExitGate / _TerminalExitDialog (US-8.5, #159 §H).
//
// Couverture :
//   - Le dialogue s'ouvre à l'appel de showTerminalExitGate ;
...
//
// Utilise le vrai `TerminalInactivityGuard` (pas de mock) pour valider la suspension
// du minuteur par comportement observable (le dialogue reste à l'écran).

import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';

import 'package:coiflink_mobile/adapters/ui/terminal/terminal_exit_gate.dart';
...
/// Dimensionne la surface en tablette pour éviter le débordement de TerminalNumericKeypad
      ...
      builder: (ctx) => Scaffold(
        body: Center(
          child: ElevatedButton(
            onPressed: () => showTerminalExitGate(ctx),
...
void main() {
  group('showTerminalExitGate — ouverture du dialogue', () {
  ...
  group('showTerminalExitGate — saisie du PIN', () {
  ...
  group('showTerminalExitGate — invariant de sécurité (§H)', () {
    testWidgets(
    ...
    testWidgets(
        'la validation ne ferme pas le dialogue (le terminal ne quitte pas son mode)',
        (tester) async {
      _setTabletSurface(tester);
      final navKey = GlobalKey<NavigatorState>();
      await tester.pumpWidget(_buildApp(navKey: navKey));
  ...
  group('showTerminalExitGate — fermeture du dialogue', () {
  ...
  group('showTerminalExitGate — suspension du minuteur d\'inactivité', () {
    testWidgets(
