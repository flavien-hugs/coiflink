// Tests widget — KioskHomeScreen (US-8.5, #159).
//
// Couverture : état chargement (spinner) ; affichage du nom du salon après succès ;
// bascule vers KioskUnavailableScreen en cas d'erreur catalogue ; le bouton Réessayer
// déclenche un nouvel appel ; le CTA Commencer pousse KioskPhoneIdentificationScreen.
// Injecte un faux gateway via KioskDeps — aucun réseau réel.
...
/// flutter_test font déborder les grands boutons et les `ListView` kiosque.
...
Widget _buildScreen(KioskDeps deps) =>
    MaterialApp(home: KioskHomeScreen(deps: deps));
...
void main() {
  group('KioskHomeScreen — état chargement', () {
  ...
  group('KioskHomeScreen — succès', () {
    ...
    testWidgets('Commencer pousse KioskPhoneIdentificationScreen', (tester) async {
      _setTabletSurface(tester);
      final gateway = _StubCatalogGateway(detail: _salon());
  ...
  group('KioskHomeScreen — erreur réseau', () {
    testWidgets('bascule vers KioskUnavailableScreen sur SalonCatalogException',
        (tester) async {
      _setTabletSurface(tester);
      final gateway =
