// Tests widget — KioskConfirmScreen (US-005, #159).
//
...
// KioskTicketNumberScreen ; erreur neutre + sélection conservée sur
// KioskQueueException (aucune navigation automatique, décision n°9) ; Modifier mon
...
KioskDeps _makeDeps(KioskQueueGateway queueGateway) => KioskDeps(
      salonId: 'salon-1',
...
void main() {
  group('KioskConfirmScreen — affichage', () {
  ...
  group('KioskConfirmScreen — confirmation', () {
    ...
    testWidgets('navigue vers KioskTicketNumberScreen sur succès', (tester) async {
      await tester.pumpWidget(_buildScreen(
      ...
      expect(find.byType(KioskTicketNumberScreen), findsOneWidget);
    });

    testWidgets('affiche une erreur neutre et reste sur l\'écran sur KioskQueueException',
        (tester) async {
      await tester.pumpWidget(_buildScreen(
        queueGateway: _StubQueueGateway(
          error: const KioskQueueException('Borne momentanément indisponible.'),
        ),
      ));
      await tester.pumpAndSettle();
      ...
      expect(find.byType(KioskTicketNumberScreen), findsNothing);
    });
  });

  group('KioskConfirmScreen — modifier', () {
